...
class DeepMojiModel_single_layer(nn.Module):
    def __init__(self, args):
        super(DeepMojiModel_single_layer, self).__init__()
        self.args = args
        self.emb_size = self.args.emb_size
        self.hidden_size = self.args.hidden_size
        self.num_classes = self.args.num_classes
        ...
        try:
            ...
            pass
        self.dense1 = nn.Linear(self.emb_size, self.num_classes)
    ...
    def hidden(self, input):
        assert self.adv_level in set([0, -1])
        if self.adv_level ==-1:
            return input
        else:
            out = self.dense1(input)
