...
from networks.deepmoji_sa import FEDA_classifier
...
def train_epoch(model, 
    ...
    for batch, p_batch in tqdm(zip(iterator, adv_iterator)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default = "/home/path/path/path/data/deepmoji/split2/")
    parser.add_argument('--model_path', type=str, default = "/home/path/path/adv_decorrelation/model/")
    parser.add_argument('--log_path', type=str, default = "/home/path/path/adv_decorrelation/logs/")
    parser.add_argument('--log_name', type=str, default = "log")
    parser.add_argument('--main_model_name', type=str, default = "deepmoji_decor_{}.pt")
    parser.add_argument('--adv_model_name', type=str, default = "deepmoji_adv_decor_{}.pt")
    parser.add_argument('--batch_size', type=int, default = 512)
    parser.add_argument('--epoch', type=int, default = 100)
    parser.add_argument('--n_hidden', type=int, default = 1)
    parser.add_argument('--dropout', type=float, default = 0.5)
    # tuning hyperparameters
    parser.add_argument('--LAMBDA_lb', type=float, default=-4)
    parser.add_argument('--LAMBDA_ub', type=float, default=4)
    parser.add_argument('--LAMBDA_times', type=int, default=10)
    parser.add_argument('--p_label_ratio', type=float, default=1.0)
    # repeating times
    ...
    main_model_path = model_path / args.main_model_name.format(args.p_label_ratio)
    adv_model_path = model_path / args.adv_model_name.format(args.p_label_ratio)
    
    args.device = device
    p_label_ratio = args.p_label_ratio

    # Load data
    train_data = DeepMojiDataset(args, data_path, "train", ratio=args.ratio, n = 100000)

    p_train_data = DeepMojiDataset(args, data_path, "train", ratio=args.ratio, n = 100000, limit_instances_ratio = p_label_ratio)

    dev_data = DeepMojiDataset(args, data_path, "dev")

    p_dev_data = DeepMojiDataset(args, data_path, "dev", limit_instances_ratio = p_label_ratio)

    test_data = DeepMojiDataset(args, data_path, "test")

    p_test_data = DeepMojiDataset(args, data_path, "test", limit_instances_ratio = p_label_ratio)
    ...
    for tune_lambda in range(args.LAMBDA_times):
        # current Lambda
        ...
        for model_repeating in range(args.repeating_times):
            f.write("$"*20+"\n")
